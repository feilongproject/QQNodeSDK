...
import { AvailableIntentsEventsEnum, GetWsParam, SessionEvents, SessionRecord, WebsocketCloseReason, WsEventDataMap, ConfigLogger, WsEventData, wsResData } from '@src/types';
...
export default class WebsocketClient extends EventEmitter {
    ...
    // 监听 ws 事件，回调类型由 WsEventDataMap 按枚举自动匹配（见 src/types/ws-events.ts）
    on(event: 'READY', listener: (data: WsEventDataMap['READY']) => void): this;
    on(event: 'ERROR', listener: (data: WsEventDataMap['ERROR']) => void): this;
    on<E extends AvailableIntentsEventsEnum>(event: E, listener: (data: WsEventDataMap[E]) => void): this;
