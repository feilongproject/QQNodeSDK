...
export class Ws {
    ...
    createListening() {
        ...
        this.ws.on('message', (data: wsResData) => {
            //    this.config.logger.info(`[CLIENT] 收到消息: ${data}`);
            ...
            if (wsRes.t === SessionEvents.READY) {
                this.config.logger.info(`[CLIENT] 鉴权通过`);
                const { d, s } = wsRes;
                const { session_id } = d;
                // 获取当前会话参数
                if (session_id && s) {
                ...
                // 第一次发送心跳
                this.config.logger.info(`[CLIENT] 发送第一次心跳`, this.heartbeatParam);
            ...
            if (wsRes.op === OpCode.HEARTBEAT_ACK || wsRes.t === SessionEvents.RESUMED) {
                ...
                this.config.logger.info('[CLIENT] 心跳校验', this.heartbeatParam);
                setTimeout(() => {
                    this.sendWs(this.heartbeatParam);
        ...
        this.ws.on('close', (data: number) => {
            this.config.logger.info('[CLIENT] 连接关闭', data);
            // 通知会话，当前已断线
            this.alive = false;
        ...
        // 监听websocket错误
        this.ws.on('error', () => {
            this.config.logger.info(`[CLIENT] 连接错误`);
    ...
    getValidIntentsType(): AvailableIntentsEventsEnum[] {
        ...
        if (!intentsIn) {
            this.config.logger.info('[CLIENT] intents不存在，默认开启全部监听事件');
            return defaultIntents;
        }
        // 如果开发者传入intents为空数组，我们默认给他开启全部监听事件
        if (intentsIn.length === 0) {
            this.config.logger.info('[CLIENT] intents为空，默认开启全部监听事件');
            return defaultIntents;
        }
        // 如果intents大于可监听数
        if (intentsIn.length > defaultIntents.length) {
            this.config.logger.info('[CLIENT] intents中的监听事件大于可监听数，仅开启有效监听事件');
        }
        // 如果intents中数据格式不对
        const typeIn = intentsIn.every((item) => typeof item === 'string');
        if (!typeIn) {
            this.config.logger.info('[CLIENT] intents中存在不合法类型，仅开启有效监听事件');
    ...
    checkShards(shardsArr: Array<number> | undefined) {
        // 没有传shards进来
        if (!shardsArr) {
            return this.config.logger.info('shards 不存在');
        }
        // 传进来的符合要求
        if (Array.isArray(shardsArr) && shardsArr.length === 2 && shardsArr[0] < shardsArr[1]) {
            return shardsArr;
        }
        return this.config.logger.info('shards 错误');
    }
    ...
    sendWs(msg: unknown) {
        try {
            ...
            this.config.logger.info(e);
        }
    }

    // 重新连接
    reconnect() {
        this.config.logger.info('[CLIENT] 等待断线重连');
    }
